...
- **Projet embarqué STM32 (I2C & UART)** → ✅ intégré (15ᵉ projet) — **ESPRIT, 2024, un semestre, équipe de 3**. J'ai mis uniquement les faits confirmés (STM32, I2C/UART, capteur ultrason, moteur, boutons, LCD, Proteus). **À me préciser si tu veux** : le **scénario réel** (qui pilote quoi en I2C/UART) et un **résultat concret**. 📷 Emplacement réservé → `public/stm32.jpg`.
- **Gestion de production — GD Tunisie** → ✅ contenu détaillé intégré (stage ingénieur, **GD Tunisie, 2025, 2 mois, individuel** ; logiciel Python : ouvriers/opérations par QR, shifts, comptage capteur, dashboard + export Excel ; validé sur PC). Vraies captures `gd*.png` conservées. *À confirmer* : langage = **Python** ?
- **GMAO** → ✅ contenu détaillé intégré (ISETN, **2022** ; logiciel de GMAO paramétré et alimenté). Le **nom du logiciel est volontairement omis** (« GMAO »), donc plus de placeholder techno. 📷 Emplacement réservé → `public/gmao.jpg`. *À préciser* : type (individuel / équipe) + durée.
